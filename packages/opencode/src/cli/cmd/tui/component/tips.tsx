...
const TIPS = [
  ...
  "CoStrict auto-handles OAuth for remote MCP servers requiring auth",
  "Add {highlight}.md{/highlight} files to {highlight}.costrict/command/{/highlight} to define reusable custom prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks in commands to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.costrict/agent/{/highlight} for specialized AI personas",
  "Configure per-agent permissions for {highlight}edit{/highlight}, {highlight}bash{/highlight}, and {highlight}webfetch{/highlight} tools",
  'Use patterns like {highlight}"git *": "allow"{/highlight} for granular bash permissions',
  'Set {highlight}"rm -rf *": "deny"{/highlight} to block destructive commands',
  'Configure {highlight}"git push": "ask"{/highlight} to require approval before pushing',
  "CoStrict auto-formats files using prettier, gofmt, ruff, and more",
  'Set {highlight}"formatter": false{/highlight} in config to disable all auto-formatting',
  "Define custom formatter commands with file extensions in config",
  "CoStrict uses LSP servers for intelligent code analysis",
  "Create {highlight}.ts{/highlight} files in {highlight}.costrict/tool/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.costrict/plugin/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent CoStrict from reading sensitive files",
  "Use {highlight}costrict-cli run{/highlight} for non-interactive scripting",
  "Use {highlight}costrict-cli run --continue{/highlight} to resume the last session",
  "Use {highlight}costrict-cli run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}costrict-cli serve{/highlight} for headless API access to CoStrict",
  "Use {highlight}costrict-cli run --attach{/highlight} to connect to a running server",
  "Run {highlight}costrict-cli upgrade{/highlight} to update to the latest version",
  "Run {highlight}costrict-cli auth list{/highlight} to see all configured providers",
  "Run {highlight}costrict-cli agent create{/highlight} for guided agent creation",
  "Use {highlight}/opencode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}costrict-cli github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/costrict-cli fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.costrict/themes/{/highlight} directory",
  ...
  "Run {highlight}costrict-cli debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} with CoStrict Zen for curated, tested models",
