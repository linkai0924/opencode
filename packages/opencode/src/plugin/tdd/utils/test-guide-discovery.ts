...
export namespace TestGuide {
  ...
  /**
   * Find project root directory by looking for .git directory or package.json
   */
  ...
  export async function load(): Promise<LoadResult> {
    ...
    try {
      ...
      log.debug("Instance.directory not available", { error: e })
      return { content: "", fileCount: 0, filePaths: [] }
    }
    ...
    let currentDir = cwd
    const ultimateStopDir = projectRoot ? path.dirname(projectRoot) : path.dirname(Global.Path.home)

    while (currentDir && currentDir !== path.dirname(currentDir)) {
      // Avoid searching in global .opencode directory
