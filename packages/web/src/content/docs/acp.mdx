---
title: ACP Support
description: Use CoStrict in any ACP-compatible editor.
---

CoStrict supports the [Agent Client Protocol](https://agentclientprotocol.com) or (ACP), allowing you to use it directly in compatible editors and IDEs.
...
To use CoStrict via ACP, configure your editor to run the `costrict-cli acp` command.

The command starts CoStrict as an ACP-compatible subprocess that communicates with your editor over JSON-RPC via stdio.
...
```json title="acp.json"
{
  "agent_servers": {
    "CoStrict": {
      "command": "/absolute/path/bin/costrict-cli",
      "args": ["acp"]
    }
  }
}
...
To open it, use the new 'CoStrict' agent in the AI Chat agent selector.
...
To use CoStrict as an ACP agent in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), add the following to your Neovim config:
...
This config sets up CodeCompanion to use CoStrict as the ACP agent for chat.
...
CoStrict works the same via ACP as it does in the terminal. All features are supported:
...
- MCP servers configured in your CoStrict config
