...
There are two types of agents in CoStrict; primary agents and subagents.
...
CoStrict comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
CoStrict comes with two built-in subagents, **General** and **Explore**. We'll look at this below.

---

## Built-in

CoStrict comes with two built-in primary agents and two built-in subagents.
...
- Global: `~/.config/costrict-cli/agent/`
- Per-project: `.costrict-cli/agent/`

```markdown title="~/.config/costrict-cli/agent/review.md"
---
...
If no temperature is specified, CoStrict uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global CoStrict config and the project specific config.
...
The model ID in your CoStrict config uses the format `provider/model-id`. For example, if you're using [CoStrict Zen](/docs/zen), you would use `costrict-cli/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `costrict-cli models` to see a list of the available models.
:::
...
:::tip
Do you have an agent you'd like to share? [Submit a PR](https://github.com/anomalyco/costrict-cli).
:::
...
```markdown title="~/.config/costrict-cli/agent/docs-writer.md"
---
...
```markdown title="~/.config/costrict-cli/agent/security-auditor.md"
---
