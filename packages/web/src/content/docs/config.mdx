---
title: Config
description: Using the CoStrict JSON config.
---

You can configure CoStrict using a JSON config file.

---

## Format

CoStrict supports both **JSON** and **JSONC** (JSON with Comments) formats.
...
For example, if your global config sets `theme: "costrict-cli"` and `autoupdate: true`, and your project config sets `model: "anthropic/claude-sonnet-4-5"`, the final configuration will include all three settings.
...
1. **Remote config** (from `.well-known/costrict-cli`) - organizational defaults
2. **Global config** (`~/.config/costrict-cli/opencode.json`) - user preferences
3. **Custom config** (`OPENCODE_CONFIG` env var) - custom overrides
4. **Project config** (`opencode.json` in project) - project-specific settings
5. **`.costrict-cli` directories** - agents, commands, plugins
...
Organizations can provide default configuration via the `.well-known/costrict-cli` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
```json title="Remote config from .well-known/costrict-cli"
{
  "mcp": {
    "jira": {
      "type": "remote",
...
Place your global CoStrict config in `~/.config/costrict-cli/opencode.json`. Use global config for user-wide preferences like themes, providers, or keybinds.
...
When CoStrict starts up, it looks for a config file in the current directory or traverse up to the nearest Git directory.
...
modes, and plugins just like the standard `.costrict-cli` directory, and should
...
The custom directory is loaded after the global config and `.costrict-cli` directories, so it **can override** their settings.

---

## Schema

The config file has a schema that's defined in [**`costrict.ai/config.json`**](https://costrict.ai/config.json).
...
You can configure server settings for the `costrict-cli serve` and `costrict-cli web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your CoStrict server.
...
You can configure the providers and models you want to use in your CoStrict config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, CoStrict tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
You can configure the theme you want to use in your CoStrict config through the `theme` option.
...
You can also define agents using markdown files in `~/.config/costrict-cli/agent/` or `.costrict-cli/agent/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, CoStrict will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`costrict-cli run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/costrict-cli/command/` or `.costrict-cli/command/`. [Learn more here](/docs/commands).
...
CoStrict will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, costrict-cli **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend CoStrict with custom tools, hooks, and integrations.

Place plugin files in `.costrict-cli/plugin/` or `~/.config/costrict-cli/plugin/`. You can also load plugins from npm through the `plugin` option.

```json title="opencode.json"
{
  "$schema": "https://costrict.ai/config.json",
  "plugin": ["costrict-cli-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict CoStrict to only use specific providers rather than disabling them one by one.
