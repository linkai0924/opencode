...
You can add external tools to CoStrict using the _Model Context Protocol_, or MCP. CoStrict supports both local and remote servers.
...
You can define MCP servers in your [CoStrict Config](https://costrict.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/costrict-cli` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
CoStrict automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, CoStrict will:
...
If the server requires authentication, CoStrict will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `costrict-cli mcp auth <server-name>`.
...
```bash
costrict-cli mcp auth my-oauth-server
```
...
```bash
costrict-cli mcp logout my-oauth-server
```

The `mcp auth` command will open your browser for authorization. After you authorize, CoStrict will store the tokens securely in `~/.local/share/costrict-cli/mcp-auth.json`.
...
costrict-cli mcp debug my-oauth-server
```
...
Your MCPs are available as tools in CoStrict, alongside built-in tools. So you can manage them through the CoStrict config like any other tool.
...
```bash
costrict-cli mcp auth sentry
```

This will open a browser window to complete the OAuth flow and connect CoStrict to your Sentry account.
