---
title: Providers
description: Using any LLM provider in CoStrict.
---

import config from "../../../config.mjs"
export const console = config.console

CoStrict uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your CoStrict config.
...
in `~/.local/share/costrict-cli/auth.json`.

---

### Config

You can customize the providers through the `provider` section in your CoStrict
config.
...
CoStrict Zen is a list of models provided by the CoStrict team that have been
tested and verified to work well with CoStrict. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with CoStrict Zen.
:::
...
It works like any other provider in CoStrict and is completely optional to use.
...
To use Amazon Bedrock with CoStrict:
...
2. **Configure authentication** using one of the following methods:
   ...
   Set one of these environment variables while running costrict-cli:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY costrict-cli

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile costrict-cli

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX costrict-cli
   ```
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for costrict-cli to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX costrict-cli
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX costrict-cli
   ```
...
5. Run the `/models` command to select a model.
   ...
   You can also add models through your costrict-cli config.
...
```bash
GITLAB_INSTANCE_URL=https://gitlab.company.com GITLAB_TOKEN=glpat-xxxxxxxxxxxxxxxxxxxx costrict-cli
```
...
{
  "$schema": "https://costrict.ai/config.json",
  "plugin": ["@gitlab/costrict-cli-gitlab-plugin"]
}
```
...
To use your GitHub Copilot subscription with costrict-cli:
...
To use Google Vertex AI with CoStrict:
...
2. Set the required environment variables:
   ...
   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id costrict-cli
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through costrict-cli, you can always configure it yourself.
...
```jsonc title="~/.config/costrict-cli/opencode.jsonc"
{
  "$schema": "https://costrict.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [costrict-cli-helicone-session](https://github.com/H2Shami/costrict-cli-helicone-session) plugin to automatically log each CoStrict conversation as a session in Helicone.

```bash
npm install -g costrict-cli-helicone-session
```
...
```json title="opencode.json"
{
  "plugin": ["costrict-cli-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each CoStrict conversation listed as a separate session.
...
You can configure costrict-cli to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility
...
You can configure costrict-cli to use local models through LM Studio.
...
You can configure costrict-cli to use local models through Ollama.
...
To use Ollama Cloud with CoStrict:
...
3. Copy the API key for use in CoStrict.
...
6. **Important**: Before using cloud models in CoStrict, you must pull the model information locally:
...
CoStrict Zen is a list of tested and verified models provided by the CoStrict team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>CoStrict Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **CoStrict Zen**.

   ```txt
   /connect
   ```

3. Enter your CoStrict API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your costrict-cli config.
...
5. You can also customize them through your costrict-cli config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' costrict-cli
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group costrict-cli
   ```
...
You can also customize models through your costrict-cli config. Here's an example of specifying provider routing order.
...
The `limit` fields allow CoStrict to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `costrict-cli auth list` to see if the credentials
...
2. For custom providers, check the costrict-cli config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your costrict-cli config.
