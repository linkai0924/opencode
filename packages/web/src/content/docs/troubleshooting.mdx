...
To debug any issues with CoStrict, you can check the logs or the session data
...
- **macOS/Linux**: `~/.local/share/costrict-cli/log/`
- **Windows**: `%USERPROFILE%\.local\share\costrict-cli\log\`

Log files are named with timestamps (e.g., `2025-01-09T123456.log`) and the most recent 10 log files are kept.

You can set the log level with the `--log-level` command-line option to get more detailed debug information. For example, `costrict-cli --log-level DEBUG`.

---

### Storage

costrict-cli stores session data and other application data on disk at:

- **macOS/Linux**: `~/.local/share/costrict-cli/`
- **Windows**: `%USERPROFILE%\.local\share\costrict-cli`
...
If you're experiencing issues with CoStrict:

1. **Report issues on GitHub**

   The best way to report bugs or request features is through our GitHub repository:

   [**github.com/anomalyco/costrict-cli/issues**](https://github.com/anomalyco/costrict-cli/issues)
...
2. **Join our Discord**
   ...
   [**costrict.ai/discord**](https://costrict.ai/discord)
...
### CoStrict won't start

1. Check the logs for error messages
2. Try running with `--print-logs` to see output in the terminal
3. Ensure you have the latest version with `costrict-cli upgrade`
...
- `costrict-cli/kimi-k2`

To figure out what models you have access to, run `costrict-cli models`
...
2. If the issue persists, try clearing your stored configuration:
   ...
   ```bash
   rm -rf ~/.local/share/costrict-cli
   ```
...
If you encounter API call errors, this may be due to outdated provider packages. costrict-cli dynamically installs provider packages (OpenAI, Anthropic, Google, etc.) as needed and caches them locally.
...
1. Clear the provider package cache:
   ...
   ```bash
   rm -rf ~/.cache/costrict-cli
   ```

2. Restart costrict-cli to reinstall the latest provider packages

This will force costrict-cli to download the most recent versions of provider packages, which often resolves compatibility issues with model parameters and API changes.
...
costrict-cli will detect if you're using Wayland and prefer `wl-clipboard`, otherwise it will try to find clipboard tools in order of: `xclip` and `xsel`.
