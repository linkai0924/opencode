---
title: TUI
description: Using the CoStrict terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

CoStrict provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running CoStrict starts the TUI for the current directory.
...
When using the CoStrict TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to CoStrict. Allows you to select from available providers and add their API keys.
...
Exit CoStrict. _Aliases_: `/quit`, `/q`
...
You can customize TUI behavior through your CoStrict config file.
